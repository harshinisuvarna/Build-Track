...
router.post("/", requirePermission(["manage_expenses", "add_entries"]), async (req, res) => {
  ...
  try {
    ...
    const transaction = new Transaction({
      createdBy: req.user._id,   // always the actual logged-in user
      title: title.trim(),
      type,
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    if (materialType !== undefined || subType !== undefined) tx.materialType = newMaterialType;
    if (_paymentStatus !== undefined) tx.paymentStatus = _paymentStatus;
