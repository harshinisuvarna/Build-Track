...
router.get("/financial", async (req, res) => {
  try {
    ...
    if (userId) {
      workersRecords = await Worker.find({
        createdBy: userId,
      });
    }
    ...
    const averagePerPhase =
      phaseCount > 0
        ? Math.round(totalExpenses / phaseCount)
        : 0;
    ...
    const latestActual =
      costPerSqftData[
        costPerSqftData.length - 1
      ];

    const chartStatus =
      latestActual > targetCost
        ? "OVER_BUDGET"
        : "ON_TRACK";
